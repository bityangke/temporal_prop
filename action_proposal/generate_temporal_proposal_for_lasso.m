function [starts, durations] = generate_temporal_proposal_for_lasso(numFrames)
% Input:
...
grid_bank   = [100];%[5, 10, 20, 30, 50, 70, 100, 200, 300, 500, 1000]; %[1, 2, 3];    % unit in frames
rand_ind = randperm(numel(grid_bank));
grid_bank = grid_bank(rand_ind);
stride_bank = [1]; %[1, 5, 10]; %[1, 2, 3];    % unit in frames
