...
for epoch=3:4 %1:7
    modelPath =  fullfile('..','data','exp_20161117_LinearRegression_256sample_128pos_denser_window_tempPool3', sprintf('net-epoch-%d.mat',epoch));
    ...
    for test_video=1:numel(imdb.images.feature_path) %60:79
        ...
        loss(test_video) = calculate_smoothL1(cdeltas(proposals.labels>0,:), targets(proposals.labels>0,:))/(numel(proposals.labels(proposals.labels>0))*4);
    end
    pos=labels>0;
    ...
    savefig(sprintf('fc6-net-epoch-%d',epoch));
    close(gcf);
    
    fprintf('Epoch %d: average loss is %.4f\n', epoch, mean(loss(loss>0)));
end