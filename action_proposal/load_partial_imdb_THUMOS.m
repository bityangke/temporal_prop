...
%         if s < prev_e+1 && s~=prev_e
%             fprintf('head overlapping GT(%d) with prev GT(%d)\n', j, j-1);
%         end
%         if e > next_s-1 && e~=next_s
%             fprintf('tail overlapping GT(%d) with next GT(%d)\n', j, j+1);
%         end
        

%         new_imdb.images.split_feature{i}.path{j} = fullfile(splitFeatDir, sprintf('%s_gt_%d.mat' , filename, j));
%         if save_flag == 1
%         current_GT_1D_feat = oneD_converted_feat(:,prev_e+1:next_s-1,:);
%         save(imdb.images.split_feature{i}.path{j}, 'current_GT_1D_feat','-v7.3');
%         end
%         new_imdb.images.split_feature{i}.gt_start_frame(j) = s-prev_e;
%         new_imdb.images.split_feature{i}.gt_end_frame(j)   = e-prev_e;
%         new_imdb.images.split_feature{i}.num_frames(j)     = (next_s-1) - (prev_e+1) + 1;
...
%         new_imdb.images.labels{count}.gt_start_frames = max(s-prev_e,1);
%         new_imdb.images.labels{count}.gt_end_frames   = min(e-prev_e,num_frames);
%         if next_s > prev_e
%             num_frames = (next_s-1) - (prev_e+1) + 1;
%         else
%             d = e - s + 1;
%             num_frames = d;
%         end
