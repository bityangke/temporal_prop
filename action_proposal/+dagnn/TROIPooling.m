...
%                 for j=1:size(cur_rois,2)
%                     [sub_sampled{1,j}, randInd{1,j}] = obj.get_feature(feature_maps(:,:,:,i), cur_rois(1,j), cur_rois(2,j), subdiv(2));
%                     pooled{1,j} = vl_nnpool(gpuArray(sub_sampled{1,j}), [poolsize 1],'pad', [0 1 0 0], 'stride', [stride 1], 'method', 'max');
%                 end
%                 obj.subsample_indice{i} = randInd;
% 
%                 output = cat(4, output, pooled{:});
...
%           function [output] = get_feature(obj, feature_map, point_start, point_end, numGrid)
        function [output, randInd] = get_feature(obj, feature_map, point_start, point_end, numGrid)
            if point_start < 1
                fprintf('point_start=%f',point_start);
            end
            if point_end > size(feature_map,2)
                fprintf('point_end=%f',point_end);
            end
            point_start = max(1, round(point_start));
            point_end = min(round(point_end), size(feature_map,2));
