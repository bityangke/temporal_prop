...
modelPath =  fullfile('.','data','exp_20161107_t3', 'net-epoch-7.mat');
...
% Load a test image(feature) and candidate bounding boxes.
    ...
    %     p_bbox(:,count) = [1 gt_bbox(i,1)+shift gt_bbox(i,2) gt_bbox(i,3) gt_bbox(i,4)]';   % shift
        ...
        p_bbox(:,count+3) = [1 gt_bbox(i,1)-3*shift gt_bbox(i,2) gt_bbox(i,3) gt_bbox(i,4)]'; % scale and shift
        p_bbox(2:5,count:count+3) = convert_xywh_to_x1y1x2y2(p_bbox(2:5,count:count+3)')';
